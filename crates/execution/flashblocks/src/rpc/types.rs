...
impl From<FlashblocksSubscriptionKind> for ExtendedSubscriptionKind {
    fn from(kind: FlashblocksSubscriptionKind) -> Self {
        Self::Flashblocks(kind)
    }
}
...
pub enum FlashblocksSubscriptionKind {
    ...
    /// Flashblock transactions have been included by the sequencer and are effectively
    /// preconfirmed.
    ///
    ...
    ///   where at least one log matches the filter. All logs are included in the response, not just
    ///   the matching ones.
