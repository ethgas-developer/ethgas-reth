...
use alloy_consensus::{BlockHeader, Receipt as AlloyReceipt, Transaction};
...
use test_utils::{
    ...
    FlashblocksAPI, FlashblocksState, PendingBlocksAPI,
    FlashblocksHarness, LocalNodeProvider, TestAccounts,
};
use reth::{
...
impl TestHarness {
    ...
    async fn new_canonical_block_without_processing(
        &mut self,
        user_transactions: Vec<TransactionSigned>,
...
async fn test_flashblock_for_new_canonical_block_clears_older_flashblocks_if_non_zero_index() {
    ...
    assert_eq!(current_block.transactions().len(), 0);
...
async fn test_non_sequential_payload_clears_pending_state() {
    ...
    // Empty flashblock with no user transactions
    assert_eq!(
        test.flashblocks
            .get_pending_blocks()
            .get_block(true)
...
async fn test_progress_canonical_blocks_without_flashblocks() {
    ...
    assert_eq!(genesis_block.body().transactions().count(), 0);
    assert!(test.flashblocks.get_pending_blocks().get_block(true).is_none());
    ...
    assert_eq!(block_one.body().transactions().count(), 1);
    assert!(test.flashblocks.get_pending_blocks().get_block(true).is_none());
    ...
    assert_eq!(block_two.body().transactions().count(), 2);
    assert!(test.flashblocks.get_pending_blocks().get_block(true).is_none());
}
