...
use alloy_rpc_types_eth::{TransactionInput, TransactionRequest, error::EthRpcErrorCode};
use test_utils::{Flashblock, Metadata};
use test_utils::{DoubleCounter, FlashblocksHarness};
use eyre::Result;
...
use test_utils::{ExecutionPayloadBaseV1, ExecutionPayloadFlashblockDeltaV1};
use serde_json::json;
    ...
    fn create_first_payload(&self) -> Flashblock {
        Flashblock {
            ...
            diff: ExecutionPayloadFlashblockDeltaV1 {
                blob_gas_used: 0,
                transactions: vec![], // No L1 deposit tx in Ethereum
                ..Default::default()
            },
            metadata: Metadata {
                block_number: 1,
                ...
                receipts: {
                    ...
                    receipts.insert(
                        ...
                        RethReceipt {
                            tx_type: Default::default(),
                            success: true,
                            cumulative_gas_used: 241279 + 44000,
                            logs: vec![
                    ...
                    receipts.insert(
                        ...
                        RethReceipt {
                            tx_type: Default::default(),
                            success: true,
                            cumulative_gas_used: 241279 + 44000 + 44000,
                            logs: vec![],
                        },
                    );
                    receipts
...
async fn test_get_pending_block() -> Result<()> {
    ...
    assert_eq!(pending_block.transactions.hashes().len(), 0); // No L1Info in Ethereum
    ...
    assert_eq!(block.transactions.hashes().len(), 4);
...
async fn test_get_transaction_by_hash_pending() -> Result<()> {
    ...
    assert_eq!(
        tx2.inner.as_eip1559().unwrap().to().unwrap(),
...
async fn test_get_transaction_receipt_pending() -> Result<()> {
    ...
    assert_eq!(receipt.gas_used(), 24000);
...
async fn test_eth_call() -> Result<()> {
    ...
    let big_spend = TransactionRequest::default()
...
async fn test_eth_estimate_gas() -> Result<()> {
    ...
    let send_estimate_gas = TransactionRequest::default()
...
async fn test_eth_subscribe_new_flashblocks() -> eyre::Result<()> {
    ...
    assert_eq!(block["transactions"].as_array().unwrap().len(), 0); // No L1 deposit in Ethereum
...
async fn test_eth_subscribe_multiple_flashblocks() -> eyre::Result<()> {
    ...
    assert_eq!(block1["transactions"].as_array().unwrap().len(), 0); // No L1 deposit
    ...
    assert_eq!(block2["transactions"].as_array().unwrap().len(), 4);
