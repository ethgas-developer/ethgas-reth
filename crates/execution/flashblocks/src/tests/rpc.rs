...
mod tests {
    ...
    async fn setup_node() -> eyre::Result<NodeContext> {
        let exec = reth::tasks::Runtime::with_existing_handle(tokio::runtime::Handle::current())
        ...
        let NodeHandle { node, node_exit_future } = NodeBuilder::new(node_config.clone())
            ...
            .extend_rpc_modules(move |ctx| {
                ...
                // a dummy url.
                let flashblocks_state = Arc::new(FlashblocksState::new(3));
                flashblocks_state.start(ctx.provider().clone());
